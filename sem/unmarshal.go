...
var (
	...
	// DisableUnmarshalTag disallow tag format at DefaultUnmarshalText.
	// Tag format starts with prefix v.
	DisableUnmarshalTag = false

	// UnmarshalText is used by Ver.UnmarshalText function.
	UnmarshalText = DefaultUnmarshalText
)

// DefaultUnmarshalText parse Ver from text.
//
// See also MaxTextLength and DisableUnmarshalTag.
func DefaultUnmarshalText(data []byte) (v Ver, err error) {
	const funcName = "DefaultUnmarshalText"
	f := formVersion
	if !DisableUnmarshalTag {
		f |= formTag
	}
